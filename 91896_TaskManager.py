...
# Information regarding the possible inputs of a user when either editting or adding a task.
...
# Gets the length of the Assignee list for getting the preselect value for "no chosen assingee."
assignee_list_length = len(task_edit_or_add_value_sets["Assignee"])

# Gets the list for different statuses before it is later updated in the add/update task choiceboxes.
status_list = []
...
task_details,title):
    """This function is repeated multiple times within itself for each of the other three steps in the add/update task process (Assignee, Priority and Status) either showing the user a choicebox for them to choose from a selection of choices or a integerbox to manually enter a priority value."""
    ...
    if repeat_iteration > 2:
        ...
        if edit_or_add == None:
            ...
            # Checks if the task's status is "Complete" and if so removes it from its assigneed user's task list.
            if task_details[4] == "Complete":
    ...
    # Checks if this is the second repeat of this function as it other wise opens the priority integerbox.
    if repeat_iteration != 1:
        # Formats the choicebox by getting all the needed information/choices.
        ...
        # As you can choose for a task to have no assigned member this adds the "Next step" option for that choicebox for users to skip this.
    ...
    # Checks to see if the task has any preexisting values and if so adds them to the choicebox details.
    if preselects != []:
        choicebox_preselect = preselects[repeat_iteration]

    # Sends the user to the integerbox if this is the second repeat of this function to get the task's priority rating or otherwise opens the choicebox with the given information.
    if repeat_iteration == 1:
    ...
    # Returns the user to the main menu if they chose "Cancel" or pressed exit.
    ...
    # Navigates the user to the previous step depending on where they are
    ...
    # Otherwise the selected value is recorded and this function is repeated    
    else:
...
def priority_integerbox(choicebox_preselect):
    """This is runs when the user is either adding or updating a task to the task dictionary. This gets the task's desired priority rating and self-validates for any invalid inputs outside of the numbers from 1 to 3."""
    ...
    # Checks if there is a preselected value for the task and either adds it to the integerbox information or not before displaying it to the user.
    ...
    # If the user pressed either "Cancel" of the exit button in the top right, this navigates the user back a step in the add/update task process (as stated in the integerbox displayed message.)
    if selected_value == None:
        selected_value = "Return to previous step"

    # Goes back to the preassigned_value_choiceboxes function with the selected value for it to be recorded and move the user to the next step.
    return selected_value

def update_task():
    """This gives the user the option of what task they wish to update before navigating them to the edit_or_add_multenterbox function for them to update the chosen task's values."""

    # Gets the list of task IDs and titles for the users to choose from which to edit.
    task_list = []
    ...
    # Checks if the user either chose "Cancel" or pressed the exit button in the top right corner to return them to the main menu.
    if task_choice == None:
        main_menu()
    
    # Otherwise, the users choice is noted and the user is directed to the edit_or_add_multenterbox.
    else:
        
        # As the choices aren't the tasks name or ID specifically this checks which value of the list was selected and loops through the task dictionary for that same index as the choice.
        task_chosen = []
        ...
        # Calls the edit_or_add_multenterbox function to begin the updating process.
        edit_or_add_multenterbox(task_multenterbox_details,"Update task",
        task_chosen)

def search(choice):
    """This function allows the user to look for a specific task or team member and get their details."""

    # Checks to see whether the user was accessing this function from the main menu or from ruturning to this from looking at a task/team member's details.
    if choice == None:
        # If accessing from main menu or exiting the task/team member choicebox the user is presented with this buttonbox.
        ...
        # Otherwise, the user is navigated forward to the task/team member choicebox.
        task_or_team_member = choice

    # Checks if the user pressed "Cancel" or "Return to main menu" and navigates them back to the main menu.
    ...
    # Runs if the user chose to search through the task dictionary.
        ...
        # Generates a choicebox with the given information for the user to choose a task from.
        task_choice = easygui.choicebox(
            "Choose a task to access:" + \
                "\n\n(Press 'Cancel' to return to search choice)",
        task_or_team_member,task_list)

        # If the user pressed "Cancel" or the exit button it returns them to the start of the search function as if they entered it through the main menu.
        if task_choice == None:
            search(None)
        
        # Otherwise, it finds (by checking for the choices index value in the possbile choices as the choice isn't the exactly the task's ID or title) the task's details.
        else:
            task_number = -1
            for task in task_list:
                task_number += 1
        ...
        # Displays the task's details in a readible format for the user to assess.
        task_details = []
        ...
        # Once the user presses "OK" it returns them back to the task choicebox.
        search("Search for task")

    # Runs if the user chose to search through the team member dictionary.
    elif task_or_team_member == "Search for team member":

        # Gets the list of team members for the user to choose from to access.
        member_list = []

        for id in team_member_dictionary:
            member_list.append(f'{id} : {team_member_dictionary[id]["Name"]}')

        # Generates a choicebox with the given information for the user to choose a team member from.
        ...
        # Otherwise, it finds (by checking for the choices index value in the possbile choices as the choice isn't the exactly the team member's ID or name) the team member's details.
        else:
            member_number = -1
        ...
        # Displays the team member's details in a readible format (without having their assigned task in the list formatting) for the user to assess.
        member_details = []
        ...
        # Once the user presses "OK" it returns them back to the team member choicebox.
        search("Search for team member")

def generate_report():
    """This function shows the user how much of each status there are within the current tasks in the task dictionary."""
    ...
    # Makes sure that it gets all the statuses and not any of the added values from the add/apdate task choiceboxes.
    for status in task_edit_or_add_value_sets["Status"]:
    ...
    # Loops through the task dictionary to get all the task's statuses and adds them up.
    for task in task_dictionary:
        repeat_iteration = -1
    ...
    # Finishes the msgbox's message by adding the statuses and their amounts within the current tasks.
    for status in status_list:
        repeat_iteration += 1  
        report_text.append(
...
def print_tasks():
    """Prints all the current tasks in a readable format for the user to assess."""
...
def main_menu():
    """This is the programs main menu which give the user the choice of where to navigate to within the program."""

    # Gets the button texts for the buttonbox.
    navgation_choices = ["Add new task","Update task", \
        "Search for task or team member","Generate report","Task summary","Quit"]

    # Displays the buttonbox.
    main_menu_navigation = easygui.buttonbox("Main navigation:", \
        "Main menu",navgation_choices)

    # Checks what the user chose and navigates them to the corrosponding funtion,
    ...
    # Otherwise, the program ends if the user pressed the exit button in the top right corner or presses the "Quit" button.
    else:
        quit()
